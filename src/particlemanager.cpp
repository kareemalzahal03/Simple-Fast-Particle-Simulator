...
int ParticleManager::getSquareID(sf::Vector2f pos) {
    ...
    if (std::isnan(pos.x) or std::isnan(pos.y)) return -1000;
...
bool ParticleManager::isValidSquareID(int squareID) {
    return 0 <= squareID and squareID < numSquares;
}
...
void ParticleManager::ParticleIterator::advanceToValid() {
    ...
    while (isEnd == false and particleIT == pm->particlesInSquare[currentSquareID].end()) {
...
bool ParticleManager::ParticleIterator::operator!=(const ParticleIterator& other) const {
    ...
    if (isEnd and other.isEnd) return false;
    else if (isEnd or other.isEnd) return true;
